...
import { Icon, IC, RagBadge, HBars, Radar } from '@/components/ui';
...
export default function ContributionPage() {
  ...
  const [highlightOutcome, setHighlightOutcome] = useState('');
  ...
  const perspectiveRadar = useMemo(() => {
    const sms = db.strategy_map.slice().sort((a, b) => a.order - b.order);
    return sms
      .map((sm) => ({
        name: sm.name,
        color: pcolor(sm.id),
        values: CONTRIB_LEVELS.map((lvl) => aggregate(list.filter((i) => i.sm_id === sm.id && levelOf(i) === lvl)).avg),
      }))
      .filter((s) => s.values.some((v) => v != null));
  }, [db.strategy_map, list]);

  const highlightRow = rows.find((r) => r.key === highlightOutcome);
  const radarSeries = highlightRow
    ? [...perspectiveRadar, { name: highlightRow.label, color: '#B3324A', values: highlightRow.cells.map((c) => c.agg.avg), dashed: true }]
    : perspectiveRadar;

  const outcomeOptions = rows.filter((r) => r.key !== '__none__');
    ...
    <>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="card-head">
          <h3>Radar Kontribusi: Bentuk Rantai Hasil (Theory of Change)</h3>
          <span className="hint">tiap sumbu = level TOC, tiap bentuk = rata-rata capaian indikator pada level tsb.</span>
        </div>
        <div className="card-pad">
          <p className="muted" style={{ fontSize: 12, marginBottom: 14 }}>
            Bentuk yang melebar merata ke 4 sumbu menandakan kontribusi indikator seimbang di sepanjang rantai hasil (Output → Intermediate Outcome → Outcome → Impact).
            Bentuk yang menciut di salah satu sumbu menandakan rantai hasil yang belum lengkap datanya atau capaiannya masih rendah pada level tersebut.
          </p>
          <Radar axes={[...CONTRIB_LEVELS]} series={radarSeries} />
          <div className="fg" style={{ maxWidth: 320, marginTop: 16 }}>
            <label>Sorot Outcome tertentu (opsional)</label>
            <select value={highlightOutcome} onChange={(e) => setHighlightOutcome(e.target.value)}>
              <option value="">— Tampilkan per Perspektif saja —</option>
