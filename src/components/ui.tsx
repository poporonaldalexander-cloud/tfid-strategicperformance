...
export const IC: Record<string, string> = {
  ...
  flow: '<circle cx="6" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><circle cx="19" cy="12" r="3"/><path d="M8.6 7.4 16.4 10.7"/><path d="M8.6 16.6 16.4 13.3"/>',
};
...
export function Radar({ axes, series, max = 1.5, rings = [0.5, 1, 1.5] }: {
  axes: string[];
  series: { name: string; color: string; values: (number | null)[]; dashed?: boolean }[];
  max?: number;
  rings?: number[];
}) {
  const W = 320, H = 300, cx = W / 2, cy = H / 2 + 6, R = 104, n = axes.length;
  const angle = (i: number) => -Math.PI / 2 + (i / n) * Math.PI * 2;
  const pt = (i: number, frac: number): [number, number] => [cx + Math.cos(angle(i)) * R * frac, cy + Math.sin(angle(i)) * R * frac];
  const hasData = series.some((s) => s.values.some((v) => v != null));
  if (!hasData) return <div className="empty muted">Tidak ada data kuantitatif.</div>;
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 20, alignItems: 'center', justifyContent: 'center' }}>
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ overflow: 'visible' }}>
        {rings.map((f, ix) => {
          const frac = Math.min(f / max, 1);
          const poly = Array.from({ length: n }, (_, i) => pt(i, frac).join(',')).join(' ');
          return <polygon key={ix} points={poly} fill="none" stroke="var(--line2)" />;
        })}
        {axes.map((_, i) => {
          const [x, y] = pt(i, 1);
          return <line key={i} x1={cx} y1={cy} x2={x} y2={y} stroke="var(--line2)" />;
        })}
        {rings.map((f, ix) => {
          const frac = Math.min(f / max, 1);
          const [x, y] = pt(0, frac);
          return <text key={'r' + ix} x={x + 4} y={y - 3} fontSize="9" fill="var(--muted2)">{Math.round(f * 100)}%</text>;
        })}
        {series.map((s, si) => {
          ...
          return (
            <g key={si}>
              <polygon points={poly} fill={s.color} fillOpacity={0.1} stroke={s.color} strokeWidth={2}
        ...
        {axes.map((a, i) => {
          const [x, y] = pt(i, 1.24);
          return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize="11" fontWeight={700} fill="var(--ink)">{a}</text>;
        })}
      </svg>
      <div className="legend">
        {series.map((s, ix) => (<span key={ix}><span className="dot" style={{ background: s.color }} />{s.name}</span>))}
      </div>
    </div>
  );
}
