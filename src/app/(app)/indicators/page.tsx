...
import { ragOf, ragHex, fmtVal, pcolor, qBars, qSpark, CONTRIB_LEVELS } from '@/lib/bsc';
...
export default function IndicatorsPage() {
  ...
  const blank = (): Indicator => ({
    ...
    indicator_type: null, direction: 'Naik', pic: session.user?.email || '', weight: null,
            ...
            <div className="modal-body">
              ...
              {tab === 'info' && (
                <div className="form-grid">
                  ...
                  <div className="fg"><label>Level Capaian (Jenis Indikator)</label>
                    <select disabled={!editable} value={edit.indicator_type || ''} onChange={(e) => set('indicator_type', e.target.value || null)}>
                      <option value="">— Belum diklasifikasikan —</option>
                      {CONTRIB_LEVELS.map((l) => <option key={l} value={l}>{l}</option>)}
                      {edit.indicator_type && !(CONTRIB_LEVELS as readonly string[]).includes(edit.indicator_type) &&
                        <option value={edit.indicator_type}>{edit.indicator_type} (nilai lama)</option>}
                    </select>
                  </div>
                  <div className="fg"><label>Bobot dalam Level (%)</label>
                    <input disabled={!editable} type="number" step="any" min={0} max={100} placeholder="kosongkan = bobot setara"
                      value={edit.weight ?? ''} onChange={(e) => set('weight', e.target.value === '' ? null : parseFloat(e.target.value))} />
                  </div>
                  ...
                  <div className="fg full"><p className="muted" style={{ fontSize: 11 }}>Bobot dipakai pada menu Peta Kontribusi untuk menghitung kontribusi berjenjang (Output → Intermediate Outcome → Outcome → Impact). Bobot dibandingkan hanya terhadap indikator lain dengan Level Capaian dan Outcome yang sama, lalu dinormalisasi menjadi 100%.</p></div>
